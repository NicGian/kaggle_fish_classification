...
def combined_model():
    ...
    image = Input(shape=(max_img_dim, max_img_dim, 3), name='image')
    # Pass it through the first convolutional layer
    x = Convolution2D(1, 1, 1, input_shape=(max_img_dim, max_img_dim, 3), border_mode='same', init='he_uniform')(image)
    x = (Activation('relu'))(x)
    #x = Dropout(0.2)(x)
    #x = (MaxPooling2D(pool_size=(2, 2), strides=(2, 2)))(x)

    x = Convolution2D(4, 3, 3, border_mode='same', init='he_uniform')(x)
    x = (Activation('relu'))(x)
    x = Dropout(0.2)(x)
    x = (MaxPooling2D(pool_size=(2, 2), strides=(2, 2)))(x)

    # Now through the second convolutional layer
    x = (Convolution2D(8, 3, 3, border_mode='same', init='he_uniform'))(x)
    x = (Activation('relu'))(x)
    x = Dropout(0.2)(x)
    x = (MaxPooling2D(pool_size=(2, 2), strides=(2, 2)))(x)
    ...
    z = Convolution2D(1, 1, 1, input_shape=(max_img_dim, max_img_dim, 3), border_mode='same', init='he_uniform')(image)
    z = (Activation('relu'))(z)    
    
    z = Convolution2D(5, 9, 9, border_mode='same', init='he_uniform')(z)
    z = (Activation('relu'))(z)
    z = Dropout(0.2)(z)
...
#    j = Convolution2D(1, 1, 1, input_shape=(max_img_dim, max_img_dim, 3), border_mode='same', init='he_uniform')(image)
#    j = (Activation('relu'))(j)    
#    
#    j = Convolution2D(4, 11, 11, border_mode='same', init='he_uniform')(j)
#    j = (Activation('relu'))(j)
#    j = Dropout(0.2)(j)
#    j = (MaxPooling2D(pool_size=(2, 2), strides=(2, 2)))(j)
#    
#    j = Convolution2D(4, 11, 11, border_mode='same', init='he_uniform')(j)
#    j = (Activation('relu'))(j)
#    j = Dropout(0.2)(j)
...
#    j = Flatten()(j)
    ...
    concatenated = merge([x, z], mode='concat')

    # Add a fully connected layer just like in a normal MLP
    x = Dense(32, activation='relu', init='he_uniform')(concatenated)
    x = Dropout(.5)(x)
